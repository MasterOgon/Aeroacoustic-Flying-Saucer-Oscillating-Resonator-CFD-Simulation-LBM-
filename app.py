...
def simulate(
    ...
    for t in range(frames):
        ...
        # ============================================================
        drag_energy = abs(
            0.006 * v_target * np.sum(mask)
        )

        if v_target >= 0.0 and last_v_target < 0.0:

            stored_impulse_up = peak_energy_up
            peak_energy_up = 0.0

            release_force = -stored_impulse_up

        elif v_target < 0.0 and last_v_target >= 0.0:

            stored_impulse_down = peak_energy_down
            peak_energy_down = 0.0

            release_force = stored_impulse_down
        ...
        if v_target < 0.0:
            ...
            if drag_energy > peak_energy_up:
                peak_energy_up = drag_energy

        else:

            if drag_energy > peak_energy_down:
                peak_energy_down = drag_energy

        net_force = release_force

        v_body += net_force / mass * 0.15
...
"""
    ...
    gr.Markdown("### Experimental eroacoustic/vibration-based airship")

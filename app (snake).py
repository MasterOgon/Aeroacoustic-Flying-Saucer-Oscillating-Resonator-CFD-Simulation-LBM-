...
# НОВЫЙ АРГУМЕНТ: wavelength
def get_dynamic_mask(X, Y, cx, cy, angle_deg, phase, mode, amplitude, dyn_freq, dyn_asym, wavelength):
    ...
    # Вращение в локальную систему координат аппарата
    Xr = Xc * cos_a + Yc * sin_a
    Yr = -Xc * sin_a + Yc * cos_a

    # Пропорции капсулы (соответствует логике HTML)
    radius = 8.0
    half_length = 36.0

    if mode == "Wave":
        # Базовая амплитуда волны
        wave_max_amp = amplitude * (dyn_freq / 15.0) * (1.0 + dyn_asym * 0.3) * 6.5
        
        # Интеграция Wavelength: вычисляем волновое число (k)
        ship_length = half_length * 2.0  # Полная длина 72.0
        k = (2 * np.pi) / (ship_length * wavelength)
        
        # Аргумент фазы использует новое волновое число k вместо жесткого 0.2
        arg = phase - Xr * k
        
        # Трохоидальное заострение фронта: dyn_asym искажает фазу.
        wave_offset = wave_max_amp * np.sin(arg + (dyn_asym * 0.5) * np.sin(arg))
        
        Yr_bent = Yr - wave_offset
    else:
        Yr_bent = Yr
    
    # Функция расстояния для капсулы со скругленными краями
    dist_sq = np.maximum(0, np.abs(Xr) - half_length)**2 + Yr_bent**2
...
def simulate(
    ...
    mass=0.65,
    damping=0.5,
    wavelength=1.0 # <--- НОВЫЙ ПАРАМЕТР ФИЗИКИ
):
    omega = 1.0 / tau
    ...
    # Переменные Linear Mode
    peak_drag = 0.0
    release_force = 0.0
    last_direction = 0.0
    
    # Переменные Wave Mode
    resistance_accumulator = 0.0
    smoothed_thrust = 0.0
    last_wave_dir = 0.0
    ...
    # Делаем маску корпуса чисто белой для наглядности изгибов
    ...
    for t in range(frames):
        # ----------------------------------------------------
        # ВЛИЯНИЕ СРЕДЫ НА РАБОТУ ДВИГАТЕЛЯ
        # ----------------------------------------------------
        # Общая механика залипания (применима к обоим режимам)
        feedback_strength = peak_drag * damping * 0.15 if osc_mode == "Linear" else (resistance_accumulator * 0.05 * damping)
        ...
        # Генерация динамической формы с передачей длины волны
        solid_mask = get_dynamic_mask(X, Y, cx, cy, attack_angle, current_phase, osc_mode, amplitude, dyn_freq, dyn_asym, wavelength)
        ...
        # Вычисление перпендикулярной скорости для аэродинамики волны
        perp_x = -thrust_dir_y
        perp_y = thrust_dir_x
        v_perp = v_x * perp_x + v_y * perp_y

        drive_x = v_target * thrust_dir_x
        drive_y = v_target * thrust_dir_y

        rho_grid = np.sum(f, axis=0)
        rho_grid = np.clip(rho_grid, 0.1, 8.0)
        ux = np.sum(f * c[:,0].reshape(9,1,1), axis=0) / (rho_grid + 1e-5)
        uy = np.sum(f * c[:,1].reshape(9,1,1), axis=0) / (rho_grid + 1e-5)

        u[0] = np.clip(ux, -0.32, 0.32)
        u[1] = np.clip(uy, -0.32, 0.32)

        f = f * (1 - omega) + omega * equilibrium(rho_grid, u)

        # Граничные условия LBM (впрыск скорости от маятника)
        u[0][solid_mask] = drive_x
        u[1][solid_mask] = drive_y
        rho_grid[solid_mask] = 1.0

        feq = equilibrium(rho_grid, u)
        for i in range(9):
        ...
        # ----------------------------------------------------
        # РАСЧЕТ ТЯГИ И ФИЗИКИ ПО РЕЖИМАМ
        # ----------------------------------------------------
        if osc_mode == "Linear":
            ...
            if drag_energy > peak_drag:
                ...
                decay_rate = min(0.98, decay_rate) 
            ...
            # ИНТЕГРАЦИЯ: Чем длиннее волна при той же частоте, тем быстрее её фазовая скорость
            wave_speed = dyn_freq * 2.5 * wavelength 
            
            forward_speed = v_x * thrust_dir_x + v_y * thrust_dir_y
            slip_speed = max(0, wave_speed - forward_speed)
            
            wave_amp_phys = amplitude * (1.0 + dyn_asym * 0.5)
            scale_phys = 0.025
            
            # Коэффициент резкости: увеличивает эффективность преобразования движения в тягу
            sharpness_factor = 1.0 + dyn_asym * 1.5
            
            # Тяга и сопротивление теперь нелинейно растут от асимметрии (формы волны)
            classical_thrust_mag = (wave_amp_phys**2) * slip_speed * 0.8 * scale_phys * sharpness_factor
            
            current_resistance = abs(v_perp) * wave_amp_phys * 0.6 * scale_phys * sharpness_factor
            resistance_accumulator += current_resistance
            resistance_accumulator *= 0.92 

            current_dir = np.sign(np.sin(current_phase))
            if current_dir != last_wave_dir and last_wave_dir != 0:
                # Импульсный выброс энергии при смене направления (хлыстовой удар)
                release_boost = resistance_accumulator * 1.8 * (1.0 + dyn_asym * 1.0)
                classical_thrust_mag += release_boost
                resistance_accumulator *= 0.35
                
            last_wave_dir = current_dir

            smoothed_thrust = smoothed_thrust * 0.88 + classical_thrust_mag * 0.12

            thrust_x = smoothed_thrust * thrust_dir_x
            thrust_y = smoothed_thrust * thrust_dir_y
            
            release_force = classical_thrust_mag    
        ...
        if view_mode == "Vorticity":
            ...
            data = np.abs(np.nan_to_num(rho_grid - 1.0, nan=0.0))
            vmax = float(np.percentile(data, 99.7)) or 1e-5
            vmin = 0.0
        ...
        telemetry_box = (
            f"TELEMETRY\n"
            f"---------------------\n"
            f"MODE     : {osc_mode.upper()}\n"
            f"DYN FREQ : {dyn_freq:.2f} Hz\n"
            f"DYN ASYM : {dyn_asym:.2f}\n"
            f"WAVELEN  : {wavelength:.2f} L\n"
            f"FORCE/THR: {release_force:.3f}\n"
            f"VELOCITY : {current_speed:.4f}\n" 
            f"COORD    : ({cx:.1f}, {cy:.1f})\n"
        )
        
        if osc_mode == "Wave":
            telemetry_box += f"ACCUMULAT: {resistance_accumulator:.3f}\n"
...
with gr.Blocks() as demo:
    ...
    with gr.Column(elem_classes="border-class"):
        ...
        with gr.Row():
            with gr.Column():
                gr.Markdown("#### Global")
                frames = gr.Slider(minimum=50, maximum=700, value=150, step=50, label="Frames")
                osc_mode = gr.Radio(["Wave", "Linear"], value="Wave", label="Oscillation Mode")
                view_mode = gr.Radio(["Vorticity", "Pressure"], value="Vorticity", label="View Mode")
        ...
        with gr.Row():
            with gr.Column():
                ...
                # НОВЫЙ ПОЛЗУНОК
                wavelength = gr.Slider(minimum=0.3, maximum=3.0, value=0.85, step=0.05, label="Wavelength (L)")
    ...
    btn.click(
        simulate, 
        inputs=[
            frames, osc_mode, view_mode, pro_gradient, attack_angle, slide_angle, 
            buoyancy, frequency, amplitude, asymmetry, gravity, tau, mass, 
            damping, wavelength  # Передаем длину волны в симуляцию
        ], 
        outputs=output
    )
